...
func runBuildServer(cmd *cobra.Command, args []string) error {
	...
	// Generate app_imports.go with blank imports for all installed apps.
	importsFile := filepath.Join(projectRoot, "cmd", "moca-server", "app_imports.go")
	if genErr := generateAppImports(projectRoot, importsFile, verbose, w); genErr != nil {
		return output.NewCLIError("Failed to generate app imports").
			WithErr(genErr).
			WithCause(genErr.Error())
	}
	defer func() { _ = os.Remove(importsFile) }() // clean up generated file after build
...
// generateAppImports scans the apps/ directory for installed apps, reads each
// app's go.mod to discover its module path, and writes a generated Go file
// with blank imports so that each app's init() fires at startup.
func generateAppImports(projectRoot, outPath string, verbose bool, w *output.Writer) error {
	appsDir := filepath.Join(projectRoot, "apps")
	appInfos, err := apps.ScanApps(appsDir)
	if err != nil {
		// No apps directory or scan error — write an empty file.
		return writeAppImportsFile(outPath, nil)
	}

	var modulePaths []string
	for _, ai := range appInfos {
		goModData, readErr := os.ReadFile(filepath.Join(ai.Path, "go.mod"))
		if readErr != nil {
			continue // skip apps without go.mod
		}
	...
	if verbose && len(modulePaths) > 0 {
		w.Print("App imports: %s", strings.Join(modulePaths, ", "))
	}

	return writeAppImportsFile(outPath, modulePaths)
}

// writeAppImportsFile writes the generated app_imports.go file.
func writeAppImportsFile(path string, modulePaths []string) error {
	var b strings.Builder
	b.WriteString("// Code generated by \"moca build server\"; DO NOT EDIT.\n")
